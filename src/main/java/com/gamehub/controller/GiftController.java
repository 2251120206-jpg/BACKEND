...
public class GiftController {
    ...
    public ResponseEntity<GiftDTO> uploadGift(
            ...
            //Phần liên quan trường id_game
            @RequestParam(required = false) Long gameId,
            Authentication authentication) throws GameException {
        logger.info("Upload gift request: DeveloperID={}, Name={}, Quantity={}, GameID={}", developerId, name, quantity, gameId);
        ...
        GiftDTO giftDTO = giftService.uploadGift(developerId, name, description, imageUrl, pointCost, quantity, gameId);
    ...
    public ResponseEntity<List<GiftDTO>> getGiftsByGameId(@PathVariable Long gameId) {
        logger.info("Fetching gifts for GameID={}", gameId);
        List<GiftDTO> gifts = giftService.getGiftsByGameId(gameId);
        return new ResponseEntity<>(gifts, HttpStatus.OK);
    }
}
